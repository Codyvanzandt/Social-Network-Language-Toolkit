...
def serialize_enter_exit_edges(edges_data):
    ...
    for enter_exit_record in edges_data:
        parsed_entries_and_exits = parse_entries_and_exits(enter_exit_record.string_key())
...
def get_entrants_and_exiters(parsed_entries_and_exits):
    ...
    for entry_or_exit, character in parsed_entries_and_exits:
        if entry_or_exit == "+":
            ...
            raise ValueError(f"Entry-Exit record {entry_or_exit}{character} must be formatted as a '+' or '-' followed by a single character name.")
...
def parse_entry_or_exit(entry_or_exit):
    ...
    entry_exit, character = stripped_entry_or_exit[0], stripped_entry_or_exit[1:]
...
def generate_edges(on_stage, entrants):
    ...
    for source_character, target_character in chain(entrants_with_each_other,entrants_with_on_stage):
